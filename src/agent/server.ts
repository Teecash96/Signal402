...
6. Directional USD_M can create a proposal only when executionEligible=true and a supported protective stop plan is declared. Wait for the first dashboard APPROVE before revalidation.
7. Re-read the account and Futures context, then call signal402_revalidate_futures_context. Revalidation creates a fresh hash bound plan and deliberately requires a second dashboard APPROVE. Call signal402_wait_for_futures_approval again.
8. After the fresh plan is approved, ask the human to type CONFIRM through signal402_confirm_futures_execution. Only then submit one MARKET order through the live runtime USD_M Futures MCP tool with symbol, side, explicit positionSide, quantity, reduceOnly, existing leverage at or below 3x, and isolated margin. Never send quoteOrderQty. Never change leverage or margin mode automatically.
9. Monitor the private Futures user stream or authenticated order status. Record submitted, order update, account update, fill, margin call, or liquidation events with signal402_record_futures_event. Reconcile the real order ID, fills, position, margin, and before and after balances.
