...
app.post('/api/futures/revalidate', async (req, res) => {
  ...
  if (proposal.status !== 'approved' || !proposal.paymentReceiptId || proposal.paymentReceiptId !== state.paymentReceiptId) {
    res.status(409).json({ success: false, error: 'Futures revalidation requires the current dashboard approved proposal and real payment receipt' });
    return;
  }
...
app.post('/api/futures/proposal', async (req, res) => {
  ...
  if (state.paymentStatus !== 'settled' || !state.paymentReceiptId || body.paymentReceiptId !== state.paymentReceiptId) {
    res.status(409).json({ success: false, error: 'A Futures proposal requires the current real B402 settlement receipt' });
    return;
  }
