...
<title>Signal402 Agent OS</title><meta id="themeColor" name="theme-color" content="#071018"><script nonce="${nonce}" src="https://cdn.tailwindcss.com"></script>
...
.theme-toggle{display:inline-flex;align-items:center;justify-content:center;gap:.4rem;min-height:38px;border:1px solid var(--line);border-radius:999px;padding:.45rem .75rem;color:var(--muted);background:rgba(17,35,45,.72);font-size:.68rem;font-weight:700;letter-spacing:.08em;line-height:1;text-transform:uppercase;transition:background .18s ease,border-color .18s ease,color .18s ease,transform .18s ease}
.theme-toggle:hover{border-color:var(--teal);color:var(--text);transform:translateY(-1px)}
body[data-theme="light"]{--bg:#f3f7f7;--surface:#ffffff;--surface-raised:#e8f1f2;--line:#c8d8db;--text:#12252c;--muted:#526a72;--teal:#087f8f;--lime:#4f7e14;--amber:#8b6500;--red:#bd3049;background:radial-gradient(circle at 86% -10%,rgba(8,127,143,.12),transparent 32rem),var(--bg);color-scheme:light}
body[data-theme="light"]:before{opacity:.32;background-image:linear-gradient(rgba(82,106,114,.1) 1px,transparent 1px),linear-gradient(90deg,rgba(82,106,114,.1) 1px,transparent 1px)}
body[data-theme="light"] .glow,body[data-theme="light"] #loginForm{background:rgba(255,255,255,.94)!important;box-shadow:0 24px 80px rgba(33,66,74,.14)!important}
body[data-theme="light"] main article .bg-ink{background:rgba(244,249,249,.96)!important;border-color:var(--line)!important}
body[data-theme="light"] main article .bg-panel{background:rgba(232,241,242,.82)!important}
body[data-theme="light"] #loginPanel{background:rgba(232,241,242,.9)!important}
body[data-theme="light"] #loginForm .bg-ink{background:#f4f9f9!important;color:var(--text)!important;border-color:var(--line)!important}
body[data-theme="light"] .theme-toggle{background:rgba(232,241,242,.95)!important;color:var(--teal)!important;border-color:var(--line)!important}
body[data-theme="light"] .bg-slate-900{background:#e7eff0!important}
body[data-theme="light"] .text-white{color:var(--text)!important}
body[data-theme="light"] .text-slate-300{color:#30474f!important}
body[data-theme="light"] .text-slate-400{color:#526a72!important}
body[data-theme="light"] .text-slate-500{color:#6c8288!important}
body[data-theme="light"] .text-slate-600{color:#809398!important}
body[data-theme="light"] .border-slate-700{border-color:#b8cbd0!important}
body[data-theme="light"] .text-cyan{color:var(--teal)!important}
body[data-theme="light"] .text-lime{color:var(--lime)!important}
body[data-theme="light"] .text-amber-300{color:#7d5c00!important}
body[data-theme="light"] .text-rose-300{color:#b52c45!important}
body[data-theme="light"] .bg-cyan{background:var(--teal)!important}
body[data-theme="light"] .bg-lime{background:var(--lime)!important}
body[data-theme="light"] [class*="border-cyan/"]{border-color:rgba(8,127,143,.34)!important}
body[data-theme="light"] [class*="bg-cyan/"]{background-color:rgba(8,127,143,.1)!important}
body[data-theme="light"] [class*="border-lime/"]{border-color:rgba(79,126,20,.34)!important}
body[data-theme="light"] [class*="bg-lime/"]{background-color:rgba(79,126,20,.1)!important}
body[data-theme="light"] [class*="border-rose/"]{border-color:rgba(181,44,69,.34)!important}
body[data-theme="light"] [class*="bg-rose/"]{background-color:rgba(181,44,69,.1)!important}
body[data-theme="light"] [class*="border-amber/"]{border-color:rgba(125,92,0,.34)!important}
body[data-theme="light"] [class*="bg-amber/"]{background-color:rgba(125,92,0,.1)!important}
...
const applyTheme=(theme,persist=true)=>{const next=theme==='light'?'light':'dark';document.body.dataset.theme=next;document.documentElement.style.colorScheme=next;if(persist){try{localStorage.setItem('signal402-theme',next)}catch(_error){}}const meta=document.getElementById('themeColor');if(meta)meta.setAttribute('content',next==='light'?'#f3f7f7':'#071018');themeButtons.forEach((button)=>{button.textContent=next==='dark'?'☼ LIGHT MODE':'☾ DARK MODE';button.setAttribute('aria-label',next==='dark'?'Switch to light mode':'Switch to dark mode');button.setAttribute('aria-pressed',String(next==='light'))})};
let savedTheme='';try{savedTheme=localStorage.getItem('signal402-theme')||''}catch(_error){};
const initialTheme=savedTheme==='light'||savedTheme==='dark'?savedTheme:(window.matchMedia?.('(prefers-color-scheme: light)').matches?'light':'dark');applyTheme(initialTheme,false);themeButtons.forEach((button)=>button.addEventListener('click',()=>applyTheme(document.body.dataset.theme==='dark'?'light':'dark')));
