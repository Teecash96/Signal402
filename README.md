...
The host sends funding as `fundingRateBps`, in basis points for one funding interval. It also sends `nextFundingTime`. Signal402 does not convert an unknown unit or guess a missing interval. After the first dashboard approval, the host must re-read the account and market data and call `signal402_revalidate_futures_context`; a stale or changed context cancels the execution path. A successful revalidation creates a fresh plan and deliberately requires a second dashboard `APPROVE`, followed by the exact `CONFIRM`, before submission.
...
Every eligible proposal also receives a 60 second execution plan. The plan binds the exact symbol, side, quantity, notional, position side, reduce only value, existing leverage, margin mode, source hashes, and expiry. A plan can be approved once and consumed once. A changed or stale plan is refused. Once Binance accepts an order, authenticated fills may arrive after the authorization TTL. Execution state is atomically persisted in `state/signal402-execution.json` so restarts do not erase in-flight reconciliation. A reconciled live order produces an ordered execution record with the plan hash, real Binance MCP tool name, order ID, fill values, before and after account state, and a SHA 256 proof hash.
